 ...
 * layout is preserved) — [MaeveIcons.codepoints] is the source of truth and is unit-tested.
 */
@Composable
fun SymIcon(
    name: String,
    ...
    val glyph = MaeveIcons.codepoints[name]
    if (glyph == null) {
        Box(Modifier.size(size))
        return
    }
    val fontSize = with(LocalDensity.current) { size.toSp() }
    Text(text = glyph.toString(), fontFamily = MaeveIcons.fontFamily, fontSize = fontSize, color = tint)
}
