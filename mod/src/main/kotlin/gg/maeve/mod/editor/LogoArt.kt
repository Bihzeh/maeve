...
/**
 * Pure geometry for the Maeve brand mark — a faceted purple gem under a 3-point gold crown —
...
object LogoArt {
    ...
    fun bands(area: Rect): List<LogoBand> {
        ...
        val bandY = gemTop - 3
        out += LogoBand(Rect(cx - gemHalf, bandY, gemHalf * 2, 2), MaevePalette.gold)
        ...
        for ((idx, px) in points.withIndex()) {
            ...
            for (j in 0 until h) {
                ...
                out += LogoBand(Rect(px - w / 2, bandY - 1 - j, w, 1), MaevePalette.gold)
            }
        }
        return out
    }
