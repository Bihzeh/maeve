...
import gg.maeve.mod.FakeHudCanvas
import gg.maeve.mod.config.Config
import gg.maeve.mod.gameCtx
...
class HudRenderControllerTest {
    private fun managerWith(fps: FpsModule) =
        ModuleManager(Config(Files.createTempDirectory("maeve-render"))).apply { register(fps) }
    ...
    @Test fun `inherited line uses the module style color`() {
        ...
        assertEquals(MaevePalette.gold, d.color)
    }
